          ...
          <div className="hidden md:flex items-center gap-8">
            {["About", "Services", "Contact"].map((item) => (
              <Link
                ...
                className="text-white/70 hover:text-cyan-400 transition-colors text-sm uppercase tracking-widest"
              >
                {item}
              </Link>
            ))}
            <button className="px-6 py-2 border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/10 rounded-full transition-all uppercase text-xs tracking-widest font-bold">
              Book a call
            </button>
          </div>
        ...
        <div className="md:hidden bg-black/95 backdrop-blur-xl absolute top-20 left-0 w-full border-b border-white/10">
            ...
            {["About", "Services", "Contact"].map((item) => (
              <Link
                key={item}
                href={`#${item.toLowerCase()}`}
                className="text-white/70 hover:text-cyan-400 text-lg"
